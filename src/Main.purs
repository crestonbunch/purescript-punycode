...
adapt delta numPoints firstTime = 
  ...
  where
    ...
    initialDelta | firstTime = delta / damp
                 | otherwise = delta / 2
...
stepOuterDecode state =
  if (state.index >= Array.length state.input) 
  then Right state
  else do
...
basicEncode { input, bias, delta, n } =
  ...
  where
    ...
    delimit s | Array.length s > 0 = Array.snoc s delim
              | otherwise = s
...
stepInnerEncode (Right state) c | c /= state.n = Right state { delta = delta' }
  where delta' | c < state.n = state.delta + 1
